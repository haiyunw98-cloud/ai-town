...
- 机构保存现金、分类库存、服务计数、当日收入/支出和客流。餐食、茶点、药品、日用品与工艺服务不能卖出负库存；余额不足时居民选择最低价基本餐食，余额也不会变为负数。饥饿和精力会影响工作、用餐、休息等活动选择，但不由模型直接改写账户。
- 关系分别保存友情、信任、亲密倾向和商业合作。已完成的普通对话双方友情 `+1`（同一组合每日最多 `+3`）；共同工作/活动为友情、信任各 `+1`；真实交易或委托使商业合作 `+1`；明确帮助或照护使友情 `+2`、信任 `+1`；明确争执使友情 `-2`。普通谈话不会自动产生恋爱数值，关系数值只由结构化变化行更新。
- 小镇暂停时不调度新活动，也不结算工作、消费、每日补货或关系变化；恢复后不会补扣暂停期间的消费。每日首次推进以上海日期的唯一键结算补货和机构固定成本，重启或补跑不会重复入账。
- 每日活动奖励与日常经济共用同一不可变账本：完成签到为 `10` 金贝参赛补贴，进入决赛额外 `20` 金贝，冠军额外 `50` 金贝；承办机构按实际餐食、药品、运输或场地服务记收入。奖励按活动与居民的唯一键结算，重复归档不会二次发放。
