...
刀人、查验、用药与投票都会以桌游演出和事件记录呈现，不表示真实伤害。观察模式会显示法官视角的夜间目标与动作；居民参赛视角仍受身份和阶段权限限制。主持使用浏览器本机中文女声，不调用云端语音服务；若设备不支持语音，字幕仍会完整继续。演出可切换 `1×`、`2×` 或跳过，跳过只影响画面节奏，不改变后台结算。桌面端采用会场与记录双栏，手机端自动切成纵向圆桌和折叠记录。
