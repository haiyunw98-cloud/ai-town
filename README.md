...
![灯塔镇本地 AI 居民社会模拟](docs/visuals/lighthouse-town-infographic-live-reference.png)
...
- **上帝观察模式**：无需操纵观察者小人靠近；点击居民即可直接交谈，也可安排工作、休息、吃饭、购物、到广场或点地图派遣。
- **日常社会模拟**：工作、收入、消费、饮食、休息、照护、合作、友情与谨慎发展的亲密关系均有结构化记录。
- **每日公共活动**：上海时间每天 `12:00–14:00` 举行安全比赛；生活类活动在主镇，大型挑战在试炼岛，奖励会进入居民金贝账户。
- **沉浸式狼人杀**：活动会进入九席圆桌会场，由中文女声系统法官从欢迎、入座、确认身份开始，按天黑、狼人、预言家、女巫、天亮、发言与投票的顺序主持；语音与字幕逐句一致。
- **原创四通道声音**：项目自带五首可审计的原创音轨，小镇白天、狼人夜晚、讨论、投票和结算会自动换曲；音乐、环境、音效和法官语音可分别调节。
- **双日报**：可导出事实流水账，也可生成带范围说明和证据边界的社会观察日志。
- **持续生活**：页面关闭后，小镇以低资源规则继续推进；重新打开后恢复实时引擎。
- **常驻纯本地后端**：站点直接运行自托管 Convex 后端，不依赖可能因云端活动上报失败而退出的开发进程；前端、后端和归档均由守护脚本自动拉起。
- **本地模型优先**：居民对话使用本地 Ollama `gemma4:12b`，嵌入使用 `mxbai-embed-large`，没有付费模型自动回退。
- **中文生活语境**：镇上没有海；中心高塔只是历史公共地标。居民主要谈工作、吃饭、邻里、买卖、公共生活与个人关系。

## 世界如何运行

![灯塔镇系统图解（人物姓名为视觉示意，实际居民以项目数据为准）](docs/visuals/lighthouse-town-infographic-readme.png)
...
同一地点被多人同时使用时，引擎会为居民分配机构周围的不同可行走位置。活动切换场地前会先安全结束冲突中的普通交谈，避免角色扎堆、反复寻路和技术错误进入赛事记录。

### 狼人杀圆桌会场

狼人杀不是一张静态结果卡。活动开始后可从观察台进入专属九席圆桌会场，居民会围桌就座，系统法官逐步主持完整流程：

1. 女法官欢迎玩家、安排九席入座并提示确认身份；
2. 天黑并关闭公开信息；
3. 狼人阵营选择安全淘汰目标；
4. 预言家查验一名居民；
5. 女巫决定是否使用解药或毒药；
...
刀人、查验、用药与投票都会以桌游演出和事件记录呈现，不表示真实伤害。观察模式会显示法官视角的夜间目标与动作；居民参赛视角仍受身份和阶段权限限制。主持使用 124 条随站点发布的 AI 合成中文女声录音，不调用云端语音服务；录音无法加载时自动回退到浏览器本机中文语音，字幕始终完整继续。演出可切换 `1×`、`2×` 或跳过，跳过只影响画面节奏，不改变后台结算。桌面端采用会场与记录双栏，手机端自动切成纵向圆桌和折叠记录。
...
`4174` 是灯塔镇专用端口，不占用常见的 Vite `5173` 端口。安装脚本会构建项目，并启动前端、纯本地 Convex 后端和研究归档服务。三个进程意外退出后都会自动重启。
...
- **上帝模式**：观察者自动常驻且不会显示成第十位居民，也不需要在地图上行走。
- **直接聊天**：点击居民后立即建立对话；切换到另一位居民时会安全结束上一段交谈。
- **居民指令**：在详情顶部安排工作、休息、吃饭、购物或到广场；先选择居民再点地图，可派遣到指定位置。
- **居民详情**：查看人生档案、照片、状态、经济与关系网络。
- **狼人杀会场**：在活动卡片点击“进入圆桌会场”；观察者可看完整法官演出，加入游戏后只显示参赛者应知信息。
- **声音**：点击底部声音按钮后会立即播放，并显示当前曲目和播放状态；可分别调整音乐、环境、音效和女法官音量，快捷键 `M` 一键静音或恢复。原创音轨及校验清单位于 `public/assets/audio/lighthouse-town/`。
...
实时引擎保持 16 毫秒模拟步长以维持移动流畅度，但每两秒才持久化一次压缩后的世界状态，减少长期运行时的高频数据库写入。聊天、指令和活动移动使用独立轻量输入计数器，不再与历史消息结果争用同一条队尾记录。研究事实仍按发生时间逐条保存并进入 IMA 归档。
