...
- 每日活动奖励与日常经济共用同一不可变账本的规则已经定义：完成签到为 `10` 金贝参赛补贴，进入决赛额外 `20` 金贝，冠军额外 `50` 金贝；承办机构将按实际餐食、药品、运输或场地服务记收入。当前生产事件流尚未接入这项日常结算调用，因此赛事状态变化不会自动记账；接入后会按活动与居民的唯一键结算，重复归档不会二次发放。
