...
export async function localChatCompletionOnce(
  ...
  return { content, retries: 0, ms: Date.now() - startedAt };
}
  ...
  const {
    ...
    if (!result.ok) {
      throw {
        retry: result.status === 429 || result.status >= 500,
        error: new Error(
          `Embedding failed with code ${result.status}: ${sanitizeProviderError(await result.text())}`,
        ),
      };
    }
  ...
  const { result: flagged } = await retryWithBackoff(async () => {
    ...
    if (!result.ok) {
      throw {
        retry: result.status === 429 || result.status >= 500,
        error: new Error(
          `Moderation failed with code ${result.status}: ${sanitizeProviderError(await result.text())}`,
        ),
      };
    }
...
export async function ollamaFetchEmbedding(text: string): Promise<{ embedding: number[] }> {
  ...
  const { result } = await retryWithBackoff(async () => {
    ...
    if (resp.status === 404) {
      const error = sanitizeProviderError(await resp.text());
